...
num_to_20 = [u'', u'один', u'два', u'три', u'четыре', u'пять', u'шесть', u'семь', u'восемь', u'девять', u'десять', u'одиннадцать', u'двенадцать', u'тринадцать', u'четырнадцать', u'пятнадцать', u'шестнадцать', u'семнадцать', u'восемнадцать', u'девятнадцать']
num_tens = [u'', u'десять', u'двадцать', u'тридцать', u'сорок', u'пятьдесят', u'шестьдесят', u'семьдесят', u'восемьдесят', u'девяносто']
num_hundreds = [u'', u'сто', u'двести', u'триста', u'четыреста', u'пятьсот', u'шестьсот', u'семьсот', u'восемьсот', u'девятьсот']
num_to_add = [u'', u'один', u'два', u'три', u'четыре', u'пять', u'шесть', u'семь', u'восемь', u'девять']

decimal_zero = u'ноль'
one_female = u'одна'
two_female = u'две'

mio_falls = [u'миллион', u'миллиона', u'миллионов']
thousand_falls = [u'тысяча', u'тысячи', u'тысяч']

decimal_part_falls = [u'целая', u'целых']
decimal_float_tens_falls = [u'десятая', u'десятих']
decimal_float_hundreds_falls = [u'сотая', u'сотых']
decimal_float_thousands_falls = [u'тисячная', u'тисячных']
decimal_float_ten_thousands_falls = [u'десятитисячная', u'десятитисячных']
...
def _make_ru_float_full_part(num_str):
    result = ''
    if num_str == '0':
        result = decimal_zero + ' ' + decimal_part_falls[1]
    else:
        num = int(num_str)
        num_parsed = _make_ru_integer(num_str, last_female=True)
...
def _make_ru_float_decimal_part(num_str):
    result = ''
    num = int(num_str)
    if num == 0:
...
def _make_ru_integer(num_str, last_female=False):
    text = ''
    strlen = len(num_str)
    if strlen > 6:
...
def _make_ru_num(num_str):
    text = ''
    if '|' in num_str:
        integer_part, float_part = num_str.split('|', 2)
        int_processed = _make_ru_float_full_part(integer_part)
        float_processed = _make_ru_float_decimal_part(float_part)
        text = int_processed + ' ' + float_processed

    else:  # process as integer number
        text = _make_ru_integer(num_str)
...
def _expand_number(m):
    num_str = m.group(0)
    return _make_ru_num(num_str)


def ru_normalize_numbers(text):
...
if __name__ == '__main__':
    texts = [u'12,53', u'1.0', u'0.3', u'текст для проверки 126897.4 был', u'текст для проверки 1596 всего', u'текст для проверки 8080808 всего', u'текст для проверки 121000.1001 всего', u'20,25', u'11,5']
    for str in texts:
        print(str, '==>', ru_normalize_numbers(str))
